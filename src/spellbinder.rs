...
use crate::{
    anger::{memorize_anger},
    danse_macabre::{memorize_death_danse, DanseMacabreCard, DanseMacabreCardKey},
    maya::{LongDate, MayaEpoch, RoundDate},
    tao_te_ching::{memorize_tao_te_ching, TaoTeChingKey},
};
...
pub type CmdRecorder = fn() -> Option<String>;
...
impl<T> CmdConveyer<T> {
    pub fn exec(&self) {
        ...
        loop {
            match record_event() {
                Some(user_input) => match craft_cmd(user_input) {
...
fn select_random(anger: &[String]) -> String {
...
pub fn try_parse_date(user_input: &str) -> Option<NaiveDate> {
    let formats = [
        "%Y-%m-%d",  // 2023-04-05
        "%Y%m%d",    // 20230405
        "%m/%d/%Y",  // 04/05/2023 (US)
...
pub fn memorize_decks() -> Decks {
    ...
    let anger = memorize_anger();
    let decks = Decks { tao, danse, anger };
    decks
}
